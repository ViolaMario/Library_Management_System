...
namespace LibraryManager.BLL
{
    public sealed class BookStore : ItemSearch<Book>
    {
        ...
        // add SearchByPerson method
        List<Book> ItemSearch<Book>.SearchByPerson(Person person)
        {
            return books.FindAll(i => i.author == person.name);
        }
        ...
        public void AddNewBook(String title, int barcode, String ISBN, String author,string Authornationality,string Authorbirthday)
        {
            if (!books.Exists(i => i.barcode == barcode))
            {
                Book book = new Book(title, barcode, ISBN, author,Authornationality,Authorbirthday);
                books.Add(book);
            }
        }
